...
int main (int argc, char *argv[]){
    ...
    if(myremainder==0){
      ...
      int ini3 = myran.int64() % pop1[71];
      ini3 += popsize0;
      dupcheck=1;
      checkcount=0;
      while(dupcheck>0){
        dupcheck=0;
        for(i=popsize0; i<popsize0+pop1[72]; i++) if(tree[72][i]==ini3) dupcheck++;
        if(dupcheck>0){
          ini3 = myran.int64() % pop1[71];
	  ini3 += popsize0;
          checkcount++;
        }
        if(checkcount>pop1[72]){
          cout << "warning: possibly infinite loop!!!\n";
          break;
        }
      }
      ...
      ngen2++;
      TypGrowth(5,0,nopop,ngen2,0.3,0.1,pop2,tree,myran.int64());
