...
int main (int argc, char *argv[]){
    ...
    if(myremainder==0){
      ...
      int ini2 = myran.int64() % pop0[68];
      int dupcheck=1;
      int checkcount=0;
      while(dupcheck>0){
        dupcheck=0;
        for(i=0; i<pop0[69]; i++) if(tree[69][i]==ini2) dupcheck++;
        if(dupcheck>0){
          ini2 = myran.int64() % pop0[68];
          checkcount++;
        }
        if(checkcount>pop0[69]){
      ...
      ngen1++;
      TypGrowth(4,8,nopop,ngen1,0.2,0.1,pop1,tree,myran.int64());
      int popsize1 = nopop;
