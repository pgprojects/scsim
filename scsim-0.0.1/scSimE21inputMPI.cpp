...
int main (int argc, char *argv[]){
    ...
    if(myremainder==0){
      ...
      for(k=0; k<tgen; k++) tree[k]= new int[500000];
      int *pop0 = new int[tgen];
      int *pop1 = new int[tgen];
      int *pop2 = new int[tgen];


      TypGrowth(3,81,nopop,ngen,0.1,0.1,pop0,tree,myran.int64());
      int popsize0=nopop;


      int ini2 = myran.int64() % pop0[11];
      int dupcheck=1;
      int checkcount=0;
      while(dupcheck>0){
        dupcheck=0;
        for(i=0; i<pop0[12]; i++) if(tree[12][i]==ini2) dupcheck++;
        if(dupcheck>0){
          ini2 = myran.int64() % pop0[11];
          checkcount++;
        }
        if(checkcount>pop0[12]){
          cout << "warning: possibly infinite loop!!!\n";
          break;
        }
      }

      int ngen1=12;
      tree[ngen1][ini2]=ini2+1;
      tree[ngen1][nopop]=ini2+1;

      ngen1++;
      TypGrowth(4,65,nopop,ngen1,0.2,0.1,pop1,tree,myran.int64());
      int popsize1=nopop;


      int ini3 = myran.int64() % pop1[23];
      ini3 += popsize0;
      dupcheck=1;
      checkcount=0;
      while(dupcheck>0){
        dupcheck=0;
        for(i=popsize0; i<popsize0+pop1[24]; i++) if(tree[24][i]==ini3) dupcheck++;
        if(dupcheck>0){
          ini3 = myran.int64() % pop1[23];
	  ini3 += popsize0;
          checkcount++;
        }
        if(checkcount>pop1[24]){
          cout << "warning: possibly infinite loop!!!\n";
          break;
        }
      }
      ...
      int ngen2=24;
      tree[ngen2][ini3]=ini3+1;
      tree[ngen2][nopop]=ini3+1;

      ngen2++;
      TypGrowth(5,50,nopop,ngen2,0.3,0.1,pop2,tree,myran.int64());
