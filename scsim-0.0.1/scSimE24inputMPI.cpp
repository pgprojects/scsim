...
int main (int argc, char *argv[]){
    ...
    if(myremainder==0){
      ...
      int ini2 = myran.int64() % pop0[28];
      int dupcheck=1;
      int checkcount=0;
      while(dupcheck>0){
        dupcheck=0;
        for(i=0; i<pop0[29]; i++) if(tree[29][i]==ini2) dupcheck++;
        if(dupcheck>0){
          ini2 = myran.int64() % pop0[28];
          checkcount++;
        }
        if(checkcount>pop0[29]){
      ...
      ngen1++;
      TypGrowth(4,48,nopop,ngen1,0.2,0.1,pop1,tree,myran.int64());
      int popsize1=nopop;


      int ini3 = myran.int64() % pop1[43];
      ini3 += popsize0;
      dupcheck=1;
      checkcount=0;
      while(dupcheck>0){
        dupcheck=0;
        for(i=popsize0; i<popsize0+pop1[44]; i++) if(tree[44][i]==ini3) dupcheck++;
        if(dupcheck>0){
          ini3 = myran.int64() % pop1[43];
	  ini3 += popsize0;
          checkcount++;
        }
        if(checkcount>pop1[44]){
          cout << "warning: possibly infinite loop!!!\n";
          break;
        }
      }
      ...
      ngen2++;
      TypGrowth(5,30,nopop,ngen2,0.3,0.1,pop2,tree,myran.int64());
